export class policyBankPage {
    ...
    clickDetailsLink() {
        return cy.get(':nth-child(2) > app-pb-policy-template-card > .card')
    }
    ...
    createPolicy(policyGroupTemplateName, name, desc, resource, invariantProperty, invariantOperator, invariantValue, control, remediation) {

        this.policylink().trigger('mouseover')
        this.policylink().should('be.visible')
        this.policylink().click()
        this.newPolicyLinkButton().click({ force: true })
        this.searchPolicyTemplate().clear()
        this.searchPolicyTemplate().type(policyGroupTemplateName)
        cy.wait(1000)
        //this.selectSearchData(policyGroupTemplateName).click()
        //cy.wait(1000)

        this.clickDetailsLink().click({ force: true })
        this.enterPolicyName().clear()
        this.enterPolicyName().type(name)
        cy.wait(1000)
        this.enterPolicyDesc().clear()
        this.enterPolicyDesc().type(desc)
        cy.wait(1000)
        this.selectSeverity().click()
        cy.wait(1000)
        this.definitionTab().click()
        cy.wait(1000)
        if (policyGroupTemplateName.includes('AWS CloudFormation')) {
            this.AWScloudFormation(resource, invariantProperty, invariantOperator, invariantValue)
        }
        this.clickControlsTab().click()
        ...
        this.remediationTab().click()
        cy.wait(1000)
        this.remediation().type(remediation)
        cy.wait(1000)
        this.finalizePolicyButton().click()
        cy.wait(3000)
    }

    AWScloudFormation(resource, invariantProperty, invariantOperator, invariantValue) {
    ...
    editPolicy() {
        ...
        this.clickDetailsLink().click()
        cy.wait(1000)
        this.editPolicyName().type(' Updated')
        cy.wait(1000)
        this.finalizePolicyButton().click()
    }
    ...
    deletePolicy(policyName) {
        this.policylink().trigger('mouseover')
        this.policylink().should('be.visible')
        this.policylink().click({ force: true })
        this.institutionMenuOnPolicybank().click({ force: true })
        this.search().clear()
        cy.reload(true)
        cy.wait(3000)
        this.search().type(policyName)
        this.selectPolicyBank(policyName).first().click()
    ...
    deletePolicyGroup(policyGroupName) {
        ...
        this.deleteDropDown().click()
        this.deleteButton().click()
        this.confirmDeleteButtons().click()
        cy.wait(10000)
    }

    PublishPolicy(policyName) {
        this.institutionMenuOnPolicybank().click({ force: true })
        cy.wait(3000)
        this.search().clear()
        cy.reload(true)
        ...
        this.search().type(policyName)
        cy.wait(1000)
        //this.selectPolicyCheckBox().first().check()
        this.selectPolicyBank(policyName).first().click()
        this.publishButton().click()
        this.inputReleaseNote().click()
        this.inputReleaseNote().type('Testing')
        this.minorButton().click()
        this.saveButton().click()
        cy.wait(5000)
    }

    DeployAsPolicyGroup(policyName, name, desc, group, attributeTag) {
        this.institutionMenuOnPolicybank().click({ force: true })
        cy.wait(3000)
        this.search().clear()
        //cy.reload(true)
        cy.wait(3000)
        this.search().type(policyName)
        this.visibleDropDown().click()
        cy.wait(2000)
        this.selectVisible().click()
        cy.wait(1000)
        this.selectedDropDown().click()
        cy.wait(1000)
        this.deployButton().click()
        this.enterPolicyGroupName().type(name)
        this.enterPolicyGroupDesc().type(desc)
        this.owningGroupDropDown().click()
        this.selectOwningGroup(group).click()
        this.attributeTagDropDown().first().click({ multiple: true })
        this.enterAttributeTag().type(attributeTag)
        this.selectAttributeTag(attributeTag).click()
        this.saveOnDeploypage().click()
        cy.wait(2000)
        this.closeAndClearSelection().click()
        cy.wait(5000)
    }

    VerifyDeployedPolicyGroup(policyGroupName) {
        this.clickBackButtonLink().click({ force: true })
        cy.wait(3000)
        this.policyGrouplink().trigger('mouseover')
